...
const types = {
  ...
  important: { 
    keyword: 'important',
    emoji: '❗️', //'&#x2757;'
    svg: '<svg xmlns="http://www.w3.org/2000/svg" width="14" height="16" viewBox="0 0 14 16"><path fill-rule="evenodd" d="M7 2.3c3.14 0 5.7 2.56 5.7 5.7s-2.56 5.7-5.7 5.7A5.71 5.71 0 0 1 1.3 8c0-3.14 2.56-5.7 5.7-5.7zM7 1C3.14 1 0 4.14 0 8s3.14 7 7 7 7-3.14 7-7-3.14-7-7-7zm1 3H6v5h2V4zm0 6H6v2h2v-2z"/></svg>',
  },
  warning: { 
    keyword: 'warning',
    emoji: '⚠️', // '&#x26A0;&#xFE0F;'
...
const _nodes = ({tagName: hName, properties: hProperties, position, children}) => {
  return {
    type: 'admonitionHTML',
    data: {
      hName,
...
const element = (tagName, classes = [], children = []) => {
  if (typeof classes === "string") classes = spaceSeparated.parse(classes)
...
module.exports = function attacher(options) {
  ...
  function blockTokenizer (eat, value, silent) {
    ...
    // consume the processed tag
    const contentString = content.join(NEWLINE)
    const add = eat(opening + food.join(NEWLINE))

    // parse the content in block mode
    const exit = this.enterBlock()
    const contentNodes = element('div', 'admonition-content', this.tokenizeBlock(contentString, now));
    exit();
    // parse the title in inline mode
    const titleNodes = this.tokenizeInline(title || keyword, now)[0];
    // create the nodes for the icon
    const settings = config.types[keyword]
    const iconNodes = config.icons === 'svg' ? nodes(settings.svg) : text(settings.emoji)

    // build the nodes for the full markup
    const admonition = (
      element('div', `admonition admonition-${keyword}`, [
        element('div', `admonition-heading`, [
          element('h5', '', [
            element('div', 'admonition-icon', [iconNodes]),
            titleNodes
          ])
        ]),
        contentNodes
